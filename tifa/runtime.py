...
class Tifa:
    ...
    def ask(self, request: str, *, verifier: dict[str, Any] | None = None, contract: TaskContract | None = None, interrupt_at: str | None = None, case_store: CaseStore | None = None, case_category: str | None = None) -> AgentResult:
            ...
            for call in calls:
                ...
                if fingerprint_counts[fingerprint] > 2:
                    ...
                    if contract and state["repairs"] < contract.max_repairs:
                        ...
                        self.messages.append({"role": "user", "content": json.dumps(asdict(feedback), ensure_ascii=False)}); store.append_trace("repair_feedback", asdict(feedback)); state["task_phase"] = TaskPhase.REPAIR.value
                    continue
                ...
                if fingerprint in self.completed_fingerprints: output, affected, ok = "reused committed tool result", [], True
                    ...
                    try:
                        registry.last_execution = None; output, affected = registry.run(call.name, call.arguments); ok = True; self.completed_fingerprints.add(fingerprint); self.memory.after_tool(call.name, call.arguments, output)
                        ...
                        category = "path_error" if "path" in str(exc).lower() else "approval_denied" if isinstance(exc, PermissionError) else "invalid_arguments" if isinstance(exc, (ValueError, KeyError, TypeError)) else "tool_timeout" if isinstance(exc, TimeoutError) else "environment_error"
        ...
        evidence = {"run_id": store.run_id, "created_at": now(), "parent_replay_id": self.parent_run_id, "task_contract": task_contract, "execution_plan": asdict(plan), "repo_snapshot": {"snapshot_id": self.workspace.fingerprint(), "commit": None, "dirty": bool(self.workspace.status), "workspace_digest": self.workspace.fingerprint()}, "context_manifest": {"policy": self.context_policy, "memory_enabled": self.memory_enabled, "selected_items": context_manifests, "dropped_items": [], "token_estimate": sum(u.get("input_tokens", u.get("input_estimate", 0)) for u in usage)}, "events": events, "checkpoints": cp_refs, "artifacts": [{k: v for k, v in a.items() if k != "content"} for a in artifacts.values()], "verifier": verification, "metrics": {"expected_state_digest": digest(state_event["payload"]), "expected_report_digest": digest(report_event["payload"]), "provider_usage": usage, "budget_usage": budget_usage, "execution_events": execution_events, "security_events": security_events}, "provenance": {"provider": self.model_client.provider, "model": self.model_client.model, "code_version": "0.5.0"}}
